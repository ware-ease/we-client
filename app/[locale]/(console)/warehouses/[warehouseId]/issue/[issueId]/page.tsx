...
const ReceiptDetail = () => {
  ...
  const issueNoteId = pathname.split('/')[4];
  ...
  const { data: issueNote } = useGoodIssueNote(true, issueNoteId);

  useEffect(() => {
    if (issueNote) {
      setFormData(issueNote);
      setSupplierName(
        requests?.find((r) => r.id === issueNote.goodRequestId)?.partnerName ||
          ''
      );
    }
  }, [issueNote, requests, setFormData]);

  const initialData: RowData[] | undefined = issueNote?.goodNoteDetails?.map(
    (detail) => ({
      id: detail.id || '',
